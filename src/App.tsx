...
export default function App() {
  ...
  if (showAdminLogin) {
    return (
      <div className="min-h-screen font-sans flex items-center justify-center p-4 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1A8FE3 0%, #0072CE 60%, #005AB5 100%)' }}>
        {/* Banner-inspired decorative ribbons */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-32 -right-32 w-96 h-96 rounded-full opacity-20" style={{ background: '#E51426' }} />
          <div className="absolute -bottom-20 -left-20 w-72 h-72 rounded-full opacity-15" style={{ background: '#80C8FF' }} />
          <div className="absolute top-1/2 -left-12 w-48 h-48 rounded-full opacity-10" style={{ background: '#004FA3' }} />
        </div>
        <motion.div
          key="adminLogin"
          initial={{ opacity: 0, scale: 0.95, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          className="max-w-md w-full bg-white p-8 sm:p-10 rounded-3xl shadow-2xl border border-white/10 text-slate-900 relative overflow-hidden"
        >
          {/* Decorative Top Gold Accent */}
          <div className="absolute top-0 left-0 right-0 h-1.5 wc-vibrant-line" />

          <div className="text-center space-y-3 mb-8">
            <div className="w-24 h-24 mx-auto flex items-center justify-center ">
              <img src="/mascot-hero.png" alt="Innoparty Mascot" className="w-full h-full object-contain" />
            </div>
            <span className="inline-block text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border" style={{ background: '#EBF5FF', color: '#0072CE', borderColor: '#0072CE' }}>
              INNOPARTY 2026 • PORTAL UTAMA
            </span>
            <h2 className="text-2xl font-black uppercase tracking-tight font-display" style={{ color: '#004FA3' }}>
              LOGIN ADMINISTRATOR
            </h2>
          ...
          <form onSubmit={handleAdminLogin} className="space-y-4">
            <div className="space-y-1.5">
              <label className="block text-xs font-black uppercase tracking-wider font-mono" style={{ color: '#0072CE' }}>
              ...
              <input
                ...
                className="w-full px-4 py-3.5 rounded-2xl border-2 bg-slate-50 focus:bg-white outline-none transition-all font-bold text-slate-900 text-sm" style={{ borderColor: '#0072CE' }}
                onFocus={e => (e.currentTarget.style.borderColor = '#0072CE', e.currentTarget.style.boxShadow = '0 0 0 4px rgba(0,114,206,0.15)')}
                onBlur={e => (e.currentTarget.style.borderColor = '#0072CE', e.currentTarget.style.boxShadow = 'none')}
                autoFocus
              />
            </div>
            ...
            <button
              type="submit"
              className="w-full py-4 rounded-2xl text-white font-black text-xs uppercase tracking-wider transition-all shadow-lg active:scale-[0.98]" style={{ background: '#E51426' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#C41121')}
  ...
  // ----------------------------------------------------------------------
  return (
    <div className="min-h-screen bg-[#F4F7FC] text-slate-900 font-sans relative overflow-x-hidden" style={{ '--tw-selection-bg': '#0B3D9B' } as React.CSSProperties}>

      {/* Background Banner Image (Flat Vector Tunas Honda without Neon) */}
      <div 
        className="fixed inset-0 pointer-events-none z-0 bg-cover bg-center bg-no-repeat bg-fixed"
        style={{ backgroundImage: "url('/flat-banner-tunas.jpg')" }}
      >
        {/* Soft Deep Blue overlay for readability on the banner bg */}
        <div className="absolute inset-0" style={{ background: 'rgba(11,61,155,0.18)', mixBlendMode: 'multiply' }} />
      </div>
