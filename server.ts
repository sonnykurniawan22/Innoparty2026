...
async function startServer() {
  ...
  app.post("/api/submit-attendance", upload.single("image"), async (req: any, res) => {
    try {
      ...
      if (targetFolderId) {
        try {
          ...
          // Log only the message so it doesn't clutter the console with stack traces
        ...
        // Fallback to base64 immediately if no folder ID
        const base64Data = `data:image/jpeg;base64,${file.buffer.toString('base64')}`;
        if (base64Data.length < 45000) {
          fileUrl = base64Data;
        } else {
      ...
      // Extract Google API specific errors if present
